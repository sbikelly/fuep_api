...
export class AuthService {
   ...
   */
  static async authenticateCandidate(
    username: string,
    password: string
  ...
  static async changePassword(
    ...
    try {
      // Get candidate with current password hash
      ...
      // If candidate has no password (first time), skip current password verification
      if (candidate.password_hash) {
      ...
      // Hash and store new password using existing PasswordUtils
  ...
  static verifyAccessToken(
    token: string
  ): { candidateId: string; iat: number; exp: number } | null {
    try {
   ...
   */
  private static generateAccessToken(candidateId: string): string {
    return jwt.sign({ candidateId, type: 'access' }, JWT_SECRET, { expiresIn: JWT_EXPIRES_IN });
  }

  /**
   * Generate JWT refresh token
   */
  private static generateRefreshToken(candidateId: string): string {
    return jwt.sign({ candidateId, type: 'refresh' }, JWT_REFRESH_SECRET, {
