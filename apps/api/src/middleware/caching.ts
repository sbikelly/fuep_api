...
class MemoryCache {
  ...
  get<T>(key: string): T | null {
    ...
    if (this.isExpired(entry)) {
      ...
      metricsStore.incrementCounter('cache_misses_total', 1, {
        operation: 'get',
        reason: 'expired',
      });
      metricsStore.setGauge('cache_size', this.cache.size);
...
export const generateAdminCacheKey = (
  operation: string,
  ...params: (string | number)[]
): string => {
...
const warmStaticDataCache = async () => {
  ...
  const paymentPurposesKey = generateCacheKey('static', 'payment-types');
  const programsKey = generateCacheKey('static', 'programs');

  const mockPaymentPurposes = [
  ...
  cacheInstances.static.set(paymentPurposesKey, mockPaymentPurposes);
