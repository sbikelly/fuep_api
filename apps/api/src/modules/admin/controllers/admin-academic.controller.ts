...
export class AdminAcademicController {
  ...
  /**
   * GET /api/admin/academic/structure - Get simplified academic structure
   */
  async getAcademicStructure(req: Request, res: Response): Promise<void> {
    try {
      this.logger.log('[AdminAcademicController] Getting simplified academic structure');

      const structure = await this.academicService.getAcademicStructure();
      ...
      this.logger.error('[AdminAcademicController] Error getting academic structure:', error);
      res.status(500).json({
        success: false,
