...
export class AdminAuditService {
  ...
  async logActionWithPerformance(
  ...
  async getAuditAnalytics(timeRange: '7d' | '30d' | '90d' | '1y' = '30d') {
    try {
      ...
      const [
        ...
        db('admin_audit_logs').where('created_at', '>=', startDate).count('* as count').first(),
        db('admin_audit_logs')
        ...
        db('admin_audit_logs')
          ...
          .select(
            db.raw(
              "AVG(CAST(JSON_EXTRACT_PATH_TEXT(details, 'performance', 'duration') as INTEGER)) as avgDuration"
            )
          )
          .first(),
      ]);
      ...
      return {
        ...
        performanceMetrics: {
          averageDuration: performanceMetrics?.avgDuration
            ? parseFloat(performanceMetrics.avgDuration as string)
            : 0,
        },
      };
    } catch (error) {
    ...
    try {
      ...
      for (const event of recentSecurityEvents) {
        ...
        if (details.security?.severity === 'critical') {
          ...
          details.security?.severity === 'medium' &&
          riskLevel !== 'critical' &&
          riskLevel !== 'high'
        ) {
          riskLevel = 'medium';
